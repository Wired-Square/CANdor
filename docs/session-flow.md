...
`ModbusScanState` (0x19) on the scan session's own channel — the same one its
...
| `ModbusScanState`   | 0x19 | Discovery sweep progress + device identification, throttled to 2 Hz (see [§ Modbus discovery](#modbus-discovery)) |
