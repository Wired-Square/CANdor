...
const CAN_TRANSMIT_KINDS: [&str; 4] = ["slcan", "gvret_tcp", "gvret_usb", "socketcan"];
...
fn get_capabilities_for_kind(kind: &str, profile: &IOProfile) -> WriterCapabilities {
    match kind {
        ...
        "gvret_tcp" | "gvret_usb" => WriterCapabilities {
            ...
            supports_extended_id: true,
            supports_rtr: false,
            available_buses: vec![0, 1, 2, 3, 4], // CAN0, CAN1, SWCAN, LIN1, LIN2
        },
...
pub async fn io_start_repeat_transmit(
    ...
    eprintln!(
        "[io_transmit] Starting repeat task {} for queue_id '{}', session '{}', interval {}ms",
        task_id, queue_id, session_id, interval_ms
    );
    ...
    let handle = tauri::async_runtime::spawn(async move {
        ...
        loop {
            ...
            if cancel_flag_clone.load(Ordering::Relaxed) {
                eprintln!(
                    "[io_transmit] Repeat task {} cancelled for queue_id '{}'",
                    task_id, queue_id_for_task
                );
                break;
            }

            // Transmit the frame through IO session
            let result = io::transmit_frame(&session_id_clone, &frame).await;
            if let Err(e) = result {
                eprintln!(
                    "[io_transmit] Repeat task {} error for queue_id '{}': {}",
                    task_id, queue_id_for_task, e
                );
                // Continue trying - don't stop on transient errors
            }
...
pub async fn io_start_repeat_group(
    ...
    let handle = tauri::async_runtime::spawn(async move {
        ...
        loop {
            ...
            if cancel_flag_clone.load(Ordering::Relaxed) {
                eprintln!(
                    "[io_transmit] Group repeat task {} cancelled for group '{}'",
                    task_id, group_id_for_task
                );
                break;
            }
            ...
            for frame in &frames {
                if let Err(e) = io::transmit_frame(&session_id_clone, frame).await {
                    eprintln!(
                        "[io_transmit] Group '{}' frame error: {}",
                        group_id_for_task, e
                    );
                    // Continue trying - don't stop on transient errors
                }
